...
engine = create_engine("mysql+pymysql://root:13723926068@localhost/mysql?charset=utf8", max_overflow=5)
...
class News163InfoPipeline(object):
    ...
    def process_item(self, item, spider):
        try:
            ...
            self.f.write(line)
            dic =dict(item)
            obc = Newstable(news_body=re.sub('[\[\]]',' ',str(dic['news_body'])),news_title=dic['news_title'],news_thread=dic['news_thread'],news_url=dic['news_url'])
            session.add(obc)
            session.commit()
        except:
            pass
